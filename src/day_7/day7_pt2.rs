...
pub fn main() {
    if let Ok(lines) = read_lines("src/day_7/input.txt") {
        ...
        let mut result_vec =
            viable_directories_to_delete(&mut dir_tree, (30000000 - (70000000 - dir_tree_size)));

        result_vec.sort();

        debug!("{}", dir_tree);
        debug!("{}", (30000000 - (70000000 - dir_tree_size)));
        debug!("{:?}", result_vec);
    }
}

fn viable_directories_to_delete(node: &FileNode, unused_space: i32) -> Vec<i32> {
    let mut sizes = Vec::new();
    if node.is_directory && node.size > unused_space {
        sizes.push(node.size);
    }
    for child in &node.children {
        sizes.extend(viable_directories_to_delete(child, unused_space));
    }
