        ...
        for line in lines {
            if let Ok(line) = line {
                if line.contains("[") {
                    ...
                    {
                        to_push = from_crate_stack.split_off(from_crate_stack.len() - num_to_move as usize).to_vec();
                    }
                    if let Some(to_crate_stack) = mutable_vec_of_crate_stacks.get_mut(to_crate - 1)
                    {
                        to_crate_stack.append(&mut to_push);
                    }
                }
            }
        }
